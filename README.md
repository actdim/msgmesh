...
The provider callback receives `msgIn` (the incoming request) and `msgOut` (the pre-initialized outgoing message for the `out` group). When a request is canceled by the caller (via `AbortSignal` in [`request()`](#cancellation)), a cancel message with `msg.status === 'canceled'` is delivered to the provider callback. The provider should check `msg.status` and handle cancellation accordingly. The bus **awaits** the callback even for cancel messages (so async cleanup completes), then skips publishing the `out` response.

A provider can also initiate cancellation itself by setting `msgOut.status = 'canceled'` before returning — the bus will skip the `out` publish and `request()` will reject with `OperationCanceledError`.

To skip handling and let another provider respond (chain of responsibility), set `msgOut.status = 'skipped'` and return.

For providers that don't need cancellation support, simply check that `msg.status !== 'canceled'` before doing work:
...
Multiple providers can register on the same channel. By default, all of them process every message independently. For a chain-of-responsibility pattern — where only one provider should handle each message — set `msgOut.status = 'skipped'` to pass the message to the next provider:

```typescript
// Provider A: handles only even sums
msgBus.provide({
    channel: 'TEST.COMPUTE_SUM',
    callback: (msg, msgOut) => {
        const sum = msg.payload.a + msg.payload.b;
        if (sum % 2 !== 0) {
            msgOut.status = 'skipped'; // pass to next provider
            return;
        }
        return sum * 10;
    },
...
// Provider B: handles only odd sums
msgBus.provide({
    channel: 'TEST.COMPUTE_SUM',
    callback: (msg, msgOut) => {
        const sum = msg.payload.a + msg.payload.b;
...
Each subscriber receives an independent copy of the message envelope (via `structuredClone` on the envelope, payload is shared by reference), so setting `msgOut.status` in one provider does not affect others.
...
- Message envelope cloned per subscriber via `structuredClone` — mutations in one subscriber do not affect others. Payload is shared by reference.
