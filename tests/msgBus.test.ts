...
describe('msgBus', () => {
    ...
    it('can use topics', async (ctx) => {
        ...
        await Promise.race([
            Promise.all([listenAll, listen1, listen2, emit1, emit2]),
            delay(timeout),
        ]);
        expect(o1).toBe(i1);
        expect(o2).toBe(i2);
    ...
    it('can multiplex', async (ctx) => {
        ...
        await Promise.race([
            Promise.all([multiplex, provide1, provide2, emit1, emit2]),
            delay(timeout),
        ]);
        expect(o1).toBe(i1);
        expect(o2).toBe(i2);
    ...
    it('can use service->msgbus adapter', async () => {
        ...
        type ApiMsgStruct = ToMsgStruct<TestApiClient, TestApiChannelPrefix, 'extraMethod'>;
